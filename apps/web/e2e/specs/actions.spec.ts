...
test.describe('actions page', () => {
  ...
  test('refuses to enqueue without an account selected', async ({ authedPage }) => {
    ...
    await authedPage.getByRole('button', { name: 'Comment' }).click();
  ...
  test('refuses to enqueue without a target URL', async ({ authedPage, api }) => {
    ...
    // The dropdown only offers ACTIVE/PAUSED accounts; a fresh row is PENDING
  ...
  test('enqueues a like batch from the UI and the jobs land', async ({ authedPage, api }) => {
    ...
    const [response] = await Promise.all([
      ...
      authedPage.getByRole('button', { name: 'Like' }).click(),
    ]);
    ...
    // The queue renders the new row: its badge plus the target.
    await expect(authedPage.getByRole('table', { name: 'Action queue' })).toContainText(TARGET);
  ...
  test('a batch over the cap is refused with the count', async ({ authedPage, api }) => {
    ...
    await authedPage.getByRole('button', { name: 'Like' }).click();
