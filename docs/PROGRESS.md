...
| P1-13  | Worker callbacks   | DONE   | —      | 2026-09 | OpenAPI `/internal/{action-callback,account-callback,heartbeat}` + `InternalHandler` (enum validation, `path.Base` screenshot sanitiser, 4096-rune truncate, 413 body limit). `JobService.RecordAttempt` (202 deferred to P3) / `RecordAuthOutcome` / `RecordHeartbeat`. `account.last_error` column. 12 handler tests green |
