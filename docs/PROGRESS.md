...
| P1-17    | SSE stream BE                 | DONE   | `feat(api)` | 2026-09 | `GET /api/stream` (text/event-stream, `event:` + JSON `data:` full entity per ADR 0010). `adapter.Hub` in-process fan-out behind `port.StreamPublisher` (service layer stays arch-clean): `Publish` never blocks the writer, an overflowing subscriber is closed and the browser EventSource reconnects. 15s keep-alive comment frames so proxies hold the connection. `AccountService` publishes `account-updated` on create/pause/resume/remove. Router: the `/api` group is auth + `act` permission; stream rides it, so analyst 403 / anonymous 401. TS helper `subscribeStream` in `packages/shared` (native EventSource, kind dispatch, reconnect-safe). 3 hub + 4 handler tests green; OpenAPI `StreamEvent` schema and regenerated Go/TS types |
