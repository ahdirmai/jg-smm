...
├── docs/             # spec set: PRD, ERD, SYSTEM_DESIGN, DESIGN_SYSTEM, DEVELOPMENT_RULE,
│                     #   DEVELOPMENT_PHASE, INFRA_ANALYST, PLATFORM_MATRIX, TICKETS,
│                     #   tickets/, platforms/, adr/, prototype/
...
Setiap keputusan arsitektur signifikan → `adr/NNNN-slug.md` (format MADR: Context, Decision, Consequences, Alternatives). Wajib ADR bila:
...
> Index: `adr/README.md` · template: `adr/0000-template.md`.

- `adr/0001-container-per-device.md` — kenapa 1 container = 1 "device" meng-host N akun (maks 1 per platform): satu device nyata memang 1 sesi/platform, hemat resource vs 1-pod-per-akun, invarian dijaga `@@unique([workerId, platform])`. Trade-off: blast radius = N akun saat pod mati (dibatasi `MAX_ACCOUNTS_PER_CONTAINER`).
- `adr/0002-playwright-vs-apify-action.md` — action pakai Playwright (Node), scrape pakai Apify.
- `adr/0003-sequential-batch-action.md` — concurrency = 1 per container.
- `adr/0004-single-team-no-workspace.md` — tanpa multi-tenant di MVP.
- `adr/0005-timescaledb-for-metrics.md` — time-series pakai TimescaleDB.
- `adr/0006-redis-list-durable-action.md` — action Redis List durable (`BLPOP`), bukan BullMQ; kontrol login Redis Pub/Sub (`control-<id>`).
- `adr/0007-shadcn-ui-design-system.md` — kenapa shadcn/ui.
- `adr/0008-go-be-sqlc-pgx.md` — BE Go + sqlc + pgx: statically typed, cold start cepat, single binary, Echo + slog cukup untuk REST + SSE + scheduler dalam 1 proses.
- `adr/0009-all-containerized-no-managed-services.md` — Postgres+Redis+MinIO self-hosted container (bukan RDS/ElastiCache/S3): murah di MVP, kontrol penuh, dev parity. Trade-off: ops DB/Redis ditanggung sendiri.
- `adr/0010-sse-over-websocket.md` — real-time pakai SSE (`EventSource`), bukan socket.io; satu arah server→browser, command via REST. Frame = entitas penuh.
- `adr/0011-worker-redis-pubsub-callback-contract.md` — kontrak worker dari `JG/automation`: worker hanya SUBSCRIBE + POST callback, tidak sentuh DB. Verdict per attempt = baris (upsert). Trade-off: Pub/Sub tanpa delivery guarantee; gap otorisasi callback ditutup setelah keluar loopback.
- `adr/0012-desired-state-worker-provisioning.md` — worker dinamis pakai desired-state reconciler (`Worker.desiredState`), bukan daftar `WORKER_IDS` statis. **Fleet default kosong; container dibuat manual dari UI**, dengan fallback auto-create saat add-akun (`PROVISION_AUTO_CREATE`). Container kosong → auto-delete. Idempotensi via `Worker.generation` + pod label. Dua pemicu: event + cron 60 dtk. Semua op di `ProvisionLog`.

## 18. On-call

- Runbook di `infra/runbooks/` per alert.
- Severity matrix di `SEVERITY.md`.
