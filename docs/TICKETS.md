...
| P2-10 | ERD akun resmi             | `OfficialAccount`, `AnalyticsSnapshot`, `AnalyticsMention`, `AnalyticsIngestRun` + enum                                             | Migration bersih; unique `(platform,handle)`; hypertable `AnalyticsSnapshot`         | M   | P0-04        | P0  |
| P2-11 | Provider adapter           | Interface `Provider` (`FetchMetrics`/`FetchMentions`) + 1 adapter placeholder; config provider                                      | Adapter pluggable; domain tak tahu provider spesifik                                 | M   | P2-10        | P0  |
| P2-12 | Analytics ingestor         | Cron 30 menit + on-demand; normalisasi → `AnalyticsSnapshot`/`AnalyticsMention`; update `lastFetchedAt`; tulis `AnalyticsIngestRun` | Snapshot idempotent per `(account, ts)`; `IngestRun` SUCCESS/PARTIAL/FAILED tercatat | L   | P2-11        | P0  |
| P2-13 | Official Account API+CRUD  | REST `/official-accounts` (list/create/update/archive) + refresh trigger; kontrak OpenAPI                                           | CRUD via OpenAPI; refresh enqueue ingest; RBAC strategist read/owner write           | M   | P2-12, P0-07 | P0  |
| P2-14 | Analytics API + SSE        | Endpoint `/analytics/overview` + `/analytics/{platform}` (KPI, tren, top post, mention) + event SSE `analytics-updated`             | Response match PLATFORM_MATRIX §2.3; p95 < 1.5 dtk                                   | M   | P2-12, P0-07 | P0  |
| P2-15 | FE Monitoring per-platform | Halaman Overview + 7 halaman per platform (KPI strip, tren, tabel, freshness badge `stale`)                                         | Layout konsisten; KPI berbeda per platform; badge stale saat >60mnt                  | L   | P2-14, P1-17 | P1  |

**Exit P2:** Scrape IG+Threads end-to-end → Post+Comment masuk DB → tampil dashboard + metric + alert + **analytics akun resmi (per platform) ter-ingest dari provider**.
