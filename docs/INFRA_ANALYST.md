...
- Cookie + KMS key: backup terenkripsi terpisah (`../infra/../infra/runbooks/dr-drill.md`).
...
| -------------------- | --------------------------------- | ----------------------------------------------- |
| Worker pod CrashLoop | `../infra/runbooks/worker-crashloop.md`    | cek log, session PVC, gen label, respawn manual |
| Pod Pending          | `../infra/../infra/runbooks/reconciler-drift.md`         | cek kuota/kapasitas/taint, scale node           |
| Postgres down        | `../infra/../infra/runbooks/postgres-unhealthy.md` | restore WAL, promote, verify                    |
| Redis OOM            | `../infra/../infra/runbooks/redis-unhealthy.md`           | cek maxmemory, eviction policy, queue integrity |
| Proxy budget         | `../infra/../infra/runbooks/proxy-budget-exceeded.md`        | matikan non-kritis, naikkan cap, audit akun     |
| DR drill             | `../infra/runbooks/dr-drill.md`            | restore ke ns terpisah, verify boot             |
| noVNC unreachable    | `../infra/../infra/runbooks/worker-crashloop.md`               | cek BE proxy, service, wsocket                  |
